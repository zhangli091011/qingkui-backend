...
from app.models import AuditLog, KnowledgeNode, LearningEvent, MistakeAsset, MistakePractice, MistakeProblem, OcrTask, new_id
...
def submit_practice(
    ...
    practice.is_correct = payload.is_correct
    practice.validation_details = payload.validation_details
    practice.status = "completed"
    practice.completed_at = datetime.now(timezone.utc)
    mistake.attempt_count += 1
    mistake.study_status = "mastered" if payload.is_correct else "reviewing"
    db.add(
        LearningEvent(
            user_id=user.id,
            node_id=mistake.knowledge_node_id,
            event_type="completed_mistake_practice",
            event_data={"mistake_id": mistake.id, "practice_id": practice.id, "passed": payload.is_correct},
        )
    )
    db.commit()
    db.refresh(practice)
