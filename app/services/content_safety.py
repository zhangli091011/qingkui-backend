...
_RULES: tuple[tuple[str, re.Pattern[str], str], ...] = (
    ...
    (
        "personal_mobile",
        re.compile(
            r"(?:(?:手机(?:号)?|联系电话|联系方式|联系我)\s*[:：]?\s*1[3-9]\d{9})|"
            r"(?<!\d)1[3-9]\d{1}[ -]\d{4}[ -]\d{4}(?!\d)",
            re.I,
        ),
        "检测到疑似个人手机号。请移除联系方式后再提交。",
    ),
    (
        "personal_identity_number",
        re.compile(r"(?<!\d)\d{6}(?:18|19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{3}[0-9Xx](?!\w)"),
        "检测到疑似身份证号码。请移除身份信息后再提交。",
    ),
    (
        "personal_email",
        re.compile(
            r"(?:我的邮箱|个人邮箱|联系我|联系方式|请发到|回复到).{0,12}"
            r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}",
            re.I | re.S,
        ),
        "检测到疑似个人邮箱。请移除联系方式后再提交。",
    ),
    (
        "personal_address",
        re.compile(
            r"(?:我家|家庭住址|家庭地址|收货地址|寄到|送到)\s*[:：]?\s*.{0,40}"
            r"(?:省|市|区|县|镇|乡|街道|路|巷|小区|栋|单元|号)",
            re.I | re.S,
        ),
        "检测到疑似家庭地址。请移除住址信息后再提交。",
    ),
    (
        "prompt_attack",
