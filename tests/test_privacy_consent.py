...
def test_registration_requires_current_privacy_notice_when_enforced(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr(settings, "privacy_consent_enforced", True)
...
def test_existing_client_remains_usable_when_privacy_enforcement_is_disabled(
    client: TestClient,
    monkeypatch,
) -> None:
    created = client.post(
        "/api/auth/register",
        json={"username": "privacy_legacy_01", "password": "privacy-pass-123"},
    )
    assert created.status_code == 201
    headers = _headers(created.json()["access_token"])

    monkeypatch.setattr(settings, "app_env", "production")
    monkeypatch.setattr(settings, "privacy_consent_enforced", False)
